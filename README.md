...
- Python 3 for the Windows/Linux bridges (dependencies in
...
1. **Choose a room.**.
    If a room is occupied the server replies with a suggested free room name.
1. **Select the audio source** — a capture device via `getUserMedia` (macOS), or
    the **WS Bridge** (Windows/Linux).
