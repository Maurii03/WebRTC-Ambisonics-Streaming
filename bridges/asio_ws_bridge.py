...
class AudioBridge:
    ...
    async def _ws_handler(self, websocket: WebSocketServerProtocol) -> None:
        ...
        # The sample clock is a single free-running broadcast timeline shared by
        # all clients. Reset it to 0 ONLY for the first client of a fresh session
        # (the set is currently empty): a clean ts=0 start that keeps timestamps
        # bounded, but WITHOUT disturbing a client that is already streaming when
        # another one joins — an unconditional reset would jump the running
        # client's timeline backwards and glitch its jitter buffer. A client that
        # joins mid-stream simply anchors to the current ts. (_accum_fill is
        # real-time callback state and is intentionally not touched from here.)
