...
Each WebSocket message is a raw binary blob containing one frame of interleaved audio:
    [s0 ch0, s0 ch1, ..., s0 ch15, s1 ch0, ..., s959 ch15]
...
- Total size  : 960 * 16 * 4 = 61,440 bytes per frame
"""
    ...
    def _audio_callback(
        ...
        while offset < frames:
            ...
            if self._accum_fill == OUTPUT_FRAME:
                # Frame complete - copy to bytes (_accum is reused immediately)
                raw: bytes = self._accum.tobytes()
                self._accum_fill = 0
