...
class ReceiverProcessor extends AudioWorkletProcessor {
  ...
  _writeFrame({ timestamp, channels }) {
    ...
    // Latency cap. Reads advance only at the audio clock, so latency the buffer
    // accumulates (a startup burst, or growth while underrunning) would otherwise
    // sit here until the 2 s ring wraps — adding delay that never drains. If
    // occupancy exceeds the prebuffer target by more than the drift tolerance,
    // snap the read pointer forward to the target: a one-time skip that keeps
    // latency low. maxDepth < bufLen, so this also caps the ring.
    // Generous cap so it catches RUNAWAY accumulation (a startup burst, growth
    // while underrunning) without fighting normal bursty delivery: only trigger
    // well above the target, and drain to a safe depth (not the bare minimum) to
    // avoid drain→underrun→drain thrashing.
    const maxDepth = this._prebuffer + 6 * this._frameSize;
    if (this._writeTs - this._readTs > maxDepth) {
      this._readTs = this._writeTs - (this._prebuffer + 2 * this._frameSize);
    }
  ...
  process(inputs, outputs) {
    ...
    // Report ring occupancy ~2×/s (telemetry to locate buffering latency).
    if ((this._depthTick = (this._depthTick || 0) + 1) >= 180) {
      this._depthTick = 0;
      this.port.postMessage({ type: 'depth', samples: this._writeTs - this._readTs });
    }
    return true;
  }
}
